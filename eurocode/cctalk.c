...
int res_modify_inhibit_status (char *recv_buf)
{
	if (check_res_flag(ACT_L_R_DISPENSING_COIN) == 1){
		...
		coin_env.inhibit_coin[1] = cctalk_env.coin_inhibit_status & 0x0002;//5角铜
		coin_env.inhibit_coin[2] = cctalk_env.coin_inhibit_status & 0x0002;//5角钢
		coin_env.inhibit_coin[3] = cctalk_env.coin_inhibit_status & 0x0004;//1角小铝
		coin_env.inhibit_coin[4] = cctalk_env.coin_inhibit_status & 0x0004;//1角钢
		coin_env.inhibit_coin[5] = cctalk_env.coin_inhibit_status & 0x0004;//1角小铝
		//OS_EXIT_CRITICAL();
